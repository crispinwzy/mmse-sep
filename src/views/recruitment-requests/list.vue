<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.q" placeholder="Search recruitment requests..." style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column label="Job Title" align="center">
        <template slot-scope="{row}">
      ...
      <el-table-column label="Requesting Dept." align="center">
        <template slot-scope="{row}">
          <span>{{ row.requestingDept }}</span>
        </template>
      </el-table-column>
      <el-table-column label="Experience Requirement" align="center">
        <template slot-scope="{row}">
          <span>{{ row.experience }} Year(s)</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="Actions" align="center" width="310" class-name="small-padding fixed-width">
        <template slot-scope="{row,$index}">
          ...
          <!-- For HR -->
          <el-button v-permission="['hr']" :disabled="row.status!=='pending'" size="mini" type="success" @click="handleModifyStatus(row, 'approve')">
            Approve
          </el-button>
          <el-button v-permission="['hr']" :disabled="row.status!=='pending'" size="mini" type="info" @click="handleModifyStatus(row, 'reject')">
            Reject
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="800px">
      <el-form ref="recruitmentForm" :rules="rules" :model="temp" label-position="left" label-width="250px" style="padding:0 20px;">
        <el-form-item label="Contract Type" prop="contactType">
          <el-radio-group v-model="temp.contactType">
            <el-radio label="Full-time" value="Full-time" />
            <el-radio label="Part-time" value="Part-time" />
          </el-radio-group>
        </el-form-item>
        ...
        <el-form-item label="Year of Experience" prop="jobTitle">
          <el-input v-model="temp.experience" />
        </el-form-item>
        <el-form-item label="Job Description" prop="jobDesc">
          <el-input v-model="temp.jobDesc" type="textarea" rows="3" />
        </el-form-item>
      </el-form>
    ...
    <!-- Detail -->
    <el-dialog v-if="selectedRow" :title="`Recruitment Request #${selectedRow.id}`" :visible.sync="detailsDialogVisible" width="600px">
      <el-descriptions :column="1" size="" border label-class-name="desc-label">
        <el-descriptions-item :span="1">
          <template slot="label">
            Contract Type
          </template>
          {{ selectedRow.contactType }}
...
import { fetchList, create, update } from '@/api/recruitment-requests'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        contactType: [{ required: true, message: 'required', trigger: 'change' }],
        requestingDept: [{ required: true, message: 'required', trigger: 'change' }],
        experience: [{ required: true, message: 'required', trigger: 'blur' }],
        jobTitle: [{ required: true, message: 'required', trigger: 'blur' }],
        jobDesc: [{ required: true, message: 'required', trigger: 'blur' }]
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    handleCreate() {
      ...
      this.$nextTick(() => {
        this.$refs['recruitmentForm'].clearValidate()
      })
    },
    createData() {
